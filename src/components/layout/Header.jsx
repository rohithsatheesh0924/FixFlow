...
const AppSwitcherMenu = ({ onClose }) => {
    ...
    // Mock Data for the App Switcher
    const APP_ITEMS = [
...
// --- Dummy Dropdown Component for generic links (Defined globally) ---
const DummyDropdownMenu = ({ name, onClose }) => {
    const menuRef = useRef(null);
    useEffect(() => {
        const handler = (event) => { if (menuRef.current && !menuRef.current.contains(event.target)) onClose(); };
...
const Header = ({ userInitials }) => {
    ...
    const renderDropdown = (openMenu) => {
        // 1. Projects Menu (AppSwitcher)
        if (openMenu === 'projects') {
            return <AppSwitcherMenu onClose={() => setOpenMenu('none')} />;
        }
        
        // 2. Generic Dropdowns (Filters, Dashboards, Teams, Plans, Apps)
                ...
                <nav className="hidden md:flex space-x-4 text-sm text-gray-700 relative">
                    {headerNavItems.map(item => {
                        ...
                        const onClickHandler = (e) => {
                            if (isMenuTrigger) {
                                e.preventDefault();
                                setOpenMenu(isOpen ? 'none' : item.type);
                            } else {
                                // Simple links navigate directly (e.g., Your work)
                                navigate(item.path);
                            }
                        };
                        ...
                        return (
                            <div key={item.name} className="relative">
                                ...
                                {isOpen && renderDropdown(openMenu)}
                            </div>
                        );
                    })}
                </nav>
